...
$stmt = $pdo->prepare('SELECT * FROM item WHERE Category="Console" AND SubCategory="Nintendo" ORDER BY Date_Added DESC');
...
$total_products_consoles = $pdo->query('SELECT * FROM item WHERE Category="Console" AND SubCategory="Nintendo"')->rowCount();

if (!$Id) {
  header('Location: login.html');
} else {
  $Id = $_GET['Id'];
  template_header('Console | Nintendo', $Id);
  

echo <<<EOT
                <div class="container containerItems mt-4 pt-4">
                  <h1>Console | Nintendo</h1>
