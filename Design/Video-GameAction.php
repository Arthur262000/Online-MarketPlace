...
$stmt = $pdo->prepare('SELECT * FROM item WHERE Category="Video-Game" AND SubCategory="Action" ORDER BY Date_Added DESC');
...
$total_products_consoles = $pdo->query('SELECT * FROM item WHERE Category="Video-Game" AND SubCategory="Action"')->rowCount();

if (!$Id) {
  header('Location: login.html');
} else {
  $Id = $_GET['Id'];
  template_header('Video Games | Action', $Id);
  

echo <<<EOT

<div class="container containerItems mt-4 pt-4">
<h1>Video Games | Action</h1>
