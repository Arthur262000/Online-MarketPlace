...
$stmt = $pdo->prepare('SELECT * FROM item WHERE Category="Video-Game" AND SubCategory="Adventure" ORDER BY Date_Added DESC');
...
$total_products_consoles = $pdo->query('SELECT * FROM item WHERE Category="Video-Game" AND SubCategory="Adventure"')->rowCount();
...
if (!$Id) {
  ...
  $Id = $_GET['Id'];
  template_header('Video Games | Adventure', $Id);
  

echo <<<EOT

<div class="container containerItems mt-4 pt-4">
<h1>Video Games | Adventure</h1>
